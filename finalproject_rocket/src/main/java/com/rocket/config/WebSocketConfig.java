...
//		registry.addEndpoint("/ws/chat").setAllowedOrigins("*").withSockJS(); //.withSockJs(); ->연결될 엔드포인트
		...
		registry.addEndpoint("/ws/chat").setAllowedOrigins(path).withSockJS();
		registry.addEndpoint("/ws/list").setAllowedOrigins(path).withSockJS(); 
		registry.addEndpoint("/ws/alarm").setAllowedOrigins(path).withSockJS();
	}
